...
void ThemTheDocGia(TREE_TDG & t, int mathe, int &i)
{
		if (t == NULL)
		{
			...
			p->thedocgia.DoubleList_MuonTra.First=NULL;
			p->left=NULL;
			p->right = NULL;
			t = p;
	...
	while(true)
		{
			...
			cout<<"Cac nut co lai : de quay lai";
...
void XuLyMuonSach(TREE_TDG& t,DauSach *DauSTam,NODE_LIST *s,Date Ngay)
{
	...
	if (t->thedocgia.DoubleList_MuonTra.First == NULL)
	{
		t->thedocgia.DoubleList_MuonTra.First = p;
		t->thedocgia.DoubleList_MuonTra.Last = p;
	}
	else
	{
		t->thedocgia.DoubleList_MuonTra.Last->right = p;
		p->left = t->thedocgia.DoubleList_MuonTra.Last;
		t->thedocgia.DoubleList_MuonTra.Last = p;
	}
}
 ...
 void TraSach(TREE_TDG &t, MCONTRO & DauS)
{
	...
	int z = 0;
	for (NODE_DOUBLELIST* k = tam->thedocgia.DoubleList_MuonTra.First; k != NULL; k = k->right)
	{
...
bool NhapDauSach(MCONTRO & DauS, string & ViTri, int toadoX, int toadoY)
{
	...
	DauS.nDS[DauS.n]->NamXuatBan = 2019;
	...
	while (true)
	{
		...
		{
			gotoXY(toadoX + 20 + DauS.nDS[DauS.n]->MaDauSach.length(), pointer1 + 1);
		...
		else if (pointer1 == toadoY + 12) // nam
		{
			...
			if (DauS.nDS[DauS.n]->NamXuatBan > 2019 || DauS.nDS[DauS.n]->NamXuatBan < 1000)
			{
...
bool LayTheLoai(MCONTRO DauS,string theloai[],int SoLuong[],int h,int i)
{
	for (int j = 0; j < h; j++)
	{
		if (DauS.nDS[i]->TheLoai == theloai[j])
		{
			SoLuong[j]++;
			return false;
		}
...
{	
	...
	for (int i = 0; i < DauS.n; i++)
	{
		if (LayTheLoai(DauS,TheLoai,SoLuong,h,i))
		{
...
while (true)
	{
		if (kbhit())
		{
			...
			{XoaManHinh(BoxHuongDan[0],BoxHuongDan[1],BoxHuongDan[2],BoxHuongDan[3]);
			HDInSach();
			XuatDanhSachDauSachCua1TheLoai(DauS,TheLoai[ptr2-15],SoLuong[ptr2-15],h,toadoX1,toadoY1);
			XoaManHinh(BoxMain[0],BoxMain[1]+1,BoxMain[2]-1,BoxMain[3]+1);
			goto a;
			}
			else if(c==27) 
...
void Xuat10SachCoSoLuongMuonMax(MCONTRO &DauS, int toadoX, int toadoY)
{
	...
	for (int h = 0; h < 10; h++)
	{
		for (int i = 0; i < DauS.n ; i++)
		{
			if (MaxDauS[h].SoLanMuon< DauS.nDS[i]->SoLanMuon && KTMax(MaxDauS, DauS.nDS[i]->MaDauSach, h) )
			{
				MaxDauS[h] = *DauS.nDS[i];
			}
		} 
	}	
	...
	do
	{
		...
		else
		{
			...
			for(int i=vitri;i<n;i++)
			{
				if((i-(vitri))==20) break;
				gotoXY(BoxMain[0] +10 , toadoY+i-(vitri));
				cout<<CanDeuChuoi(a[i],20)<<char(179)<<CanDeuChuoi(str[i],45);
			}
			gotoXY(BoxMain[0]+10,pointer);
			HighLight();
			cout<<CanDeuChuoi(a[pointer-(BoxMain[2]+3)+vitri],20)<<char(179)<<CanDeuChuoi(str[pointer-(BoxMain[2]+3)+vitri],45);
			i=pointer-(BoxMain[2]+3)+vitri;
			Normal();
			gotoXY(BoxMain[0]+20+x.length(),BoxMain[2]);
			ShowCur(1);
			if(kbhit)
			{
				...
				else if (c == 32 && x.length() < 40 && x.length() != 0 && x[x.length()- 1] != ' ')
				{
...
void TimSach(MCONTRO DauS)
{
	...
	gotoXY(x,y+4);
	cout<<" The loai     :                              ";	
	gotoXY(BoxMain[0]+150,BoxMain[2]-2);
